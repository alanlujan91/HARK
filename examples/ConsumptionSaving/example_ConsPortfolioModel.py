...
init_discrete_share[
    "vFuncBool"
] = True  # Have to actually construct value function for this to work
...
StickyType.vFuncFxd = [
    StickyType.solution[t].vFuncFxd for t in range(StickyType.T_cycle)
]
print(
...
# %%
print(
    "Value function over market resources when the agent CAN'T adjust, by current share:"
)
M = np.linspace(1.0, 50.0, 200)
for s in np.linspace(0.0, 1.0, 21):
    C = StickyType.vFuncFxd[0](M, s * np.ones_like(M))
    plt.plot(M, C)
plt.xlim(0.0, 50.0)
plt.show()
